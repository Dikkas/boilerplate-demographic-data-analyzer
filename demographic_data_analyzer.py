...
def calculate_demographic_data(print_data=True):
    ...
    for country in countries:
        ...
        if len(people) == 0:
            continue
        rich_count = len(people[people['salary'] == '>50K'])
        percent_rich = (rich_count / len(people)) * 100
        rich_percentage_by_country[country] = percent_rich
        
    # What country has the highest percentage of people that earn >50K?
    highest_earning_country = max(rich_percentage_by_country, key=rich_percentage_by_country.get)
    highest_earning_country_percentage = round(rich_percentage_by_country[highest_earning_country], 1)
